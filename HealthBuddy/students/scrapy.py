...
def process_response_soup(soup, c):
    for link in soup.select('.TableText a'):
        ...
        # )
        # s.save()
        sql = 'INSERT INTO students_students(roll, username, name, program, dept, hall, room, blood_group, gender, hometown) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);'

        c.execute(sql, (roll, username, name, program, dept, hall, room, blood_group, gender, hometown))
